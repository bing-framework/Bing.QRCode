...
public class QRCoderQRCodeService : QRCodeServiceBase, IQRCodeService
{
    ...
    {
        var data = _generator.CreateQrCode(param.Content, _level);
        var qrcode = new QC.QRCode(data);
        using var bitmap = qrcode.GetGraphic(param.Size, param.Foreground, param.Background, GetLogo(param.Logo), iconBorderWidth: 20, drawQuietZones: param.DrawBorder);
    ...
    /// 获取Logo文件
    /// </summary>
    /// <param name="logo">LOGO图片路径</param>
    protected virtual Bitmap GetLogo(string logo)
    {
        if (string.IsNullOrWhiteSpace(logo))
            return null;
        return Image.FromFile(logo) as Bitmap;
    }
}
